...
- The `piper` CLI (`piper/cli.py`) is a Typer app; `piper/runner.py` dispatches each run by execution mode. There are exactly three, one `ExecutionMode` value each, selected by a single `--mode` option: `blocking` (`client.execute`), `durable` (`client.start` + `client.wait_for_result`, attended), and `detached` (`client.start` only — durable but unattended, resumed via `piper runs status|result|wait <id>`). Detached is *not* a flag on top of durable: keeping the modes mutually exclusive is what makes `_dispatch` a flat exhaustive match with no invalid combination to reject. It branches on mode explicitly rather than using the SDK's `start_and_wait` self-healing one-liner, because teaching the mode difference is the point.
