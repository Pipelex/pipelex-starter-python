...
- **Breaking (CLI): `--detach` is gone; detached is now a third `--mode` value.** Detachment was a sub-choice of durable mode expressed as an independent boolean, so the two options could spell a combination that does not exist (`--mode blocking --detach`) and `_dispatch` had to reject it at runtime. The three execution modes are now three mutually exclusive values of one option — `--mode durable` (default), `--mode detached`, `--mode blocking` — which makes the invalid state unrepresentable and reduces the dispatch to a flat exhaustive match. Use `--mode detached` where you used `--detach`; `PIPELEX_EXECUTION_MODE=detached` works too.
