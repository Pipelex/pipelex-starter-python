...
The typed models are **not hand-written**: they are generated from the `.mthds` bundles into `piper/generated/` (stamped, with a `codegen.lock` per method) by the hosted API's codegen route, reached through the same `pipelex-sdk` the runs go through — so regenerating needs your API key and no local Pipelex runtime. Edit a bundle → `make codegen` regenerates the models → `make codegen-check` verifies offline that nothing is stale or hand-edited. The test suite runs that same check over every generated tree, so a client left stale is caught by CI too, with no local Pipelex runtime. See [docs/codegen.md](docs/codegen.md).
